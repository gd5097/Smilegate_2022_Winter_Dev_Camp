...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'cksgnl523!@#'
...
def index():
    ...
    if request.method == 'POST':
        ...
        existing_url_data = conn.execute('SELECT id FROM urls WHERE original_url=?', (url,))
        existing_url_data_fetch_result = existing_url_data.fetchone()

        if existing_url_data_fetch_result is None:
            url_data = conn.execute('INSERT INTO urls (original_url) VALUES(?)', (url,))
            conn.commit()
            conn.close()
            url_id = url_data.lastrowid
        else:
            url_id = existing_url_data_fetch_result[0]
